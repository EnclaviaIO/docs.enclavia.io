...
- [cargo install](#cargo-install): the published [`enclavia-cli`](https://crates.io/crates/enclavia-cli) crate on crates.io, if you already have a Rust toolchain.
- [Nix](#nix): one command, no system dependencies to install.
...
The CLI is published on crates.io as [`enclavia-cli`](https://crates.io/crates/enclavia-cli); the installed binary is still called `enclavia` (the crate's `[[bin]]` name).
...
`~/.cargo/bin` should already be on your `$PATH` if you installed Rust via rustup. To upgrade later, re-run the same command: `cargo install` rebuilds when a newer version has been published.
